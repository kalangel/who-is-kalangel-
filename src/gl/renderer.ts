...
function direct(p: number, t: number, fov0: number): Shot {
  ...
  let skyD = mix(1.05, 1.5, S(0.2, 0.5, p))
  skyD = mix(skyD, 3.0, S(0.62, 0.8, p))
  skyD = mix(skyD, 4.6, S(0.78, 0.91, p))

  let skyG = mix(0.55, 0.25, S(0.1, 0.3, p))
  skyG = mix(skyG, 1.0, S(0.55, 0.72, p))
  skyG = mix(skyG, 1.35, S(0.8, 0.92, p))
  ...
  starR = emix(starR, 0.0022, S(0.63, 0.86, p))
  const starA = S(0.5, 0.545, p) * (1 - S(0.8, 0.868, p))
  ...
  if (p > 0.9) {
    ...
    // Расстояние идёт по логарифму: линейное читается рывком в конце.
  ...
  const bloom = mix(0.30, 0.46, S(0.25, 0.45, p)) + flash * 0.04 + S(0.93, 1.0, p) * 0.25
  const aberr = 0.12 + S(0.4, 0.52, p) * 0.5 + S(0.93, 1.0, p) * 0.45
...
export function createRenderer(canvas: HTMLCanvasElement, mark: Mark): Renderer {
  ...
  function measureFov() {
    ...
    //   • не мельче шести пикселей ЦЕЛИ рендера — иначе клетка не
    //     разрешается, мип гасит слой, и на просевшем разрешении созвездие
    //     просто исчезает с экрана;
    //   • не мельче пяти пикселей ЭКРАНА — иначе звёзды сливаются, и вместо
    //     рисунка из точек получается светящаяся наклейка.
    // Отсюда шаг считается от цели, а не от окна.
    const density = rh / h // = dpr × текущий масштаб рендера
    const cell = Math.max(6, 5 * density)
