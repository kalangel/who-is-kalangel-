...
function direct(p: number, t: number, fov0: number): Shot {
  ...
  // ---- звезда-гигант ------------------------------------------------------
  // Единица здесь — узкая сторона кадра целиком: в шейдере uv нормируется на
  // неё, и по этой стороне координата идёт от −0.5 до 0.5. Радиус 0.4 — это
  // диаметр в 0.8 узкой стороны, те самые три четверти кадра.
  let starR = emix(2.0, 0.4, S(0.505, 0.61, p))
  starR = emix(starR, 0.0022, S(0.61, 0.84, p))
  // Гигант проявляется уже за белым полем, а не в нём. Начало на 0.5 давало
  // его поверхность поверх ещё лензированного кадра, когда белое набрало
  // только половину: в кадре одновременно стояли дыра и звезда.
  const starA = S(0.522, 0.556, p) * (1 - S(0.8, 0.87, p))
  ...
  const focus = 0.88 * S(0.72, 0.86, p) * (1 - S(0.9, 0.97, p))

  // Дыра выключается из кадра ровно в тот момент, когда белое поле стоит на
  // единице (0.522–0.535). Порог 0.508 приходился на белое в шесть десятых —
  // способ съёмки менялся на глазах у зрителя: лензированный кадр пропадал
  // разом, и это и был тот самый скачок в середине.
  let hole = p < 0.528
  let swirl = 0
  let lens = 1
  ...
  // Возврат начинается раньше и идёт быстрее. К 0.6 небо уже плотное, а
  // аберрация ещё держалась на четырёх десятых: каждая звезда в кадре стояла
  // красно-синей парой. Разгон нужен падению, а не звёздным актам.
  aberr = mix(aberr, 0.1, S(0.52, 0.60, p))
  // Финальный разгон скромный. На полном закрученные дырой звёзды у краёв
  // кадра разбирались на красную и синюю точки — то самое стекло из §6.5a,
  // только в последнем акте, где на него и смотрят дольше всего.
  aberr += S(0.93, 1.0, p) * 0.2
...
export function createRenderer(canvas: HTMLCanvasElement, mark: Mark): Renderer {
  ...
  function measureFov() {
    ...
    // Единица кадра — узкая сторона: шейдер нормирует uv на неё, чтобы объект
    // вписывался в экран любой пропорции, а не только в ландшафтный.
    const base = Math.min(w, h)

    const wm = document.querySelector<HTMLElement>('#wordmark')
    const width = wm?.getBoundingClientRect().width ?? base * 0.35
    const starR = Math.max(width / 1.6, 1)
    fov0 = ((SHADOW_B / 15.5) * base) / starR
    ...
    const visHalfX = 0.5 * (w / base) * FOV_SKY
    ...
    const rBase = Math.min(rw, rh) // узкая сторона цели рендера
    const density = rBase / base // = dpr × текущий масштаб рендера
    const cell = Math.max(5, 3.2 * density)
    markDensity = rBase / (cell * FOV_SKY)
